...
int main(void)
{
    ...
    rc = flash_device_base(rsp.br_flash_dev_id, &flash_base);
    if (rc != 0) {
        BOOT_LOG_ERR("flash_device_base failed with %d\n", rc);
        FIH_PANIC;
    }
    BOOT_LOG_INF("Booting from flash id: %d at offset 0x%x, base addr: 0x%x", rsp.br_flash_dev_id, rsp.br_image_off, flash_base);
